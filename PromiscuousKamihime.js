...
function onGameApp() {
	...
	//free man robot資料
	let _isFreeManSearching = false;//避免重復執行
	...
	async function initBattleObservers() {
		try {
			...
			if (kh._statisticsRepository) {
				...
				repoPrototype.logDown = function(nodeKey, normalizedPos, count) {};
				const originalLogUp = repoPrototype.logUp;
				repoPrototype.logUp = function(nodeKey, normalizedPos, holdDuration) {};
				const originalPickHotData = repoPrototype.pickHotData;
				repoPrototype.pickHotData = function(targetKey, limit) {};
			...
			//日誌,把紀錄全攔了
			const logger = kh.createInstance("logger");
			if (logger) {
				const loggerPrototype = Object.getPrototypeOf(logger);
				loggerPrototype.reportBattleActionRequestRaw = loggerPrototype.reportBattleActionRequest;
				loggerPrototype.reportBattleActionRequest = function(sceneInstanceId, data) {};
				loggerPrototype.reportBattleCharacterStatusRaw = loggerPrototype.reportBattleCharacterStatus;
				loggerPrototype.reportBattleCharacterStatus = function(sceneInstanceId, data) {};
				loggerPrototype.reportBattleExitRaw = loggerPrototype.reportBattleExit;
				loggerPrototype.reportBattleExit = function(sceneInstanceId, data) {}; 
				loggerPrototype.reportBattleScenarioRaw = loggerPrototype.reportBattleScenario;
				loggerPrototype.reportBattleScenario = function(sceneInstanceId, data) {}; 
				loggerPrototype.reportBattleStartRaw = loggerPrototype.reportBattleStart;
				loggerPrototype.reportBattleStart = function(sceneInstanceId, data) {}; 
				loggerPrototype.reportBattleStatusRaw = loggerPrototype.reportBattleStatus;
				loggerPrototype.reportBattleStatus = function(sceneInstanceId, data) {}; 
				loggerPrototype.reportBattleStatusEffectsRaw = loggerPrototype.reportBattleStatusEffects;
				loggerPrototype.reportBattleStatusEffects = function(sceneInstanceId, data) {}; 
				loggerPrototype.reportCustomBattleDataRaw = loggerPrototype.reportCustomBattleData;
				loggerPrototype.reportCustomBattleData = function(sceneInstanceId, data, customCategory) {};
				loggerPrototype.reportCommunicationErrorRaw = loggerPrototype.reportCommunicationError;
				loggerPrototype.reportCommunicationError = function(reqUrlObj, responseObj, retryInfo) {};
				loggerPrototype.reportGlobalErrorRaw = loggerPrototype.reportGlobalError;
				loggerPrototype.reportGlobalError = function(errorEvent) {};
				loggerPrototype.reportJsExceptionRaw = loggerPrototype.reportJsException;
				loggerPrototype.reportJsException = function(errorEvent) {};
				loggerPrototype.reportPageTransitionRaw = loggerPrototype.reportPageTransition;
				loggerPrototype.reportPageTransition = function(pageInfo) {};
				loggerPrototype.reportPopupTransitionRaw = loggerPrototype.reportPopupTransition;
				loggerPrototype.reportPopupTransition = function(actionType, popupWidget) {};
				loggerPrototype.reportPubsubEventRaw = loggerPrototype.reportPubsubEvent;
				loggerPrototype.reportPubsubEvent = function(eventName, data) {};
	...
	*/
	...
	async function executeDeveloperTests() {
		try {
			...
			//列出所有轉蛋資訊
			//await fetchAllGachaPools();
			//取得隊伍資料
			//await fetchPartyDetails();
			//查詢表情符號
			//await fetchOwnedStamps();
			//查詢關卡資料
			//await fetchElementQuest();
			//await fetchMaterialQuest();
			//await fetchAccessoryQuest();
			...
			// //公會留言板
			// const unionRes = await _httpClient.get({url: `${kh.env.urlRoot}/a_unions/me/chats`}, "unblock");
	...
	/**
	 * @description 根據傳入的 epic_id 查詢該關卡的詳細資訊
	 * @param {number} epicId - 史詩關卡的 ID
	 */
	async function fetchEpicQuestDetail(epicId) {
		try {
			if (!_httpClient) {debugLog("HTTP connection not initialized");return;}
	...
	/**
	 * @description 解鎖所有史詩關卡
	 */
	async function unlockAllEpicQuests() {
		try {
			//取得所有史詩關卡狀態列表
			const epicRes = await _httpClient.get({ url: `${kh.env.urlRoot}/a_epic_subject` }, "unblock");
			...
			//未解鎖關卡
			const lockedQuests = allQuests.filter(quest => quest.is_unlocked === false);
	...
	async function clearEpicQuest(epicData) {
		try {
			...
			if (unclearedQuest.type === "epic_story") {
				...
				return await handleEpicBattle(unclearedQuest.quest_id, epicData.targetElement, epicData.partyId);
			}
			return 0;
		} catch (error) {
	 ...
	 * @param {number} targetElement - 克制敵人的我方屬性
	 * @param {number} partyId - 我方出戰隊伍ID
	 * @returns {number} 進入戰鬥關卡回傳2, 未知錯誤回傳3
	 */
	async function handleEpicBattle(questId, targetElement, partyId) {
		try {
			//取得剋制屬性與對應隊伍
			...
			if (!isSuccess) {debugLog("failed to launch raid battle. aborting");return 3;}
			return 2;
		} catch (error) {
	...
	/**
	 * @description 一次性取得所有屬性對應的隊伍，建立字典快取
	 * @returns {Promise<object>} 回傳 { map: { 屬性: 隊伍ID }, defaultPartyId: 預設隊伍ID, defaultElement: 預設隊伍屬性 }
	 */
	async function buildElementPartyMap() {
		const result = {map: {}, defaultPartyId: null, defaultElement: null};
		try {
			const apiAParties = kh.createInstance("apiAParties");
			const response = await apiAParties.getList();
			const partyIds = response.body?.a_party_ids?.slice(0, 84);
			...
			for (const id of partyIds) {
				...
				const deck = deckData.body?.deck;
				if (deck) {
					//記錄當前選中的預設隊伍 (作為幻屬性或查無克制屬性時的備案)
					if (deck.selected === true && result.defaultPartyId === null) {
						result.defaultPartyId = deck.a_party_id;
						result.defaultElement = deck.element_type;
					}
					//如果該屬性還沒被記錄過，則記錄下來
					if (result.map[deck.element_type] === undefined) {
						result.map[deck.element_type] = deck.a_party_id;
					}
				}
				//檢查是否已經找齊 0~5 屬性以及預設隊伍
				const foundBasicElements = Object.keys(result.map).filter(k => k >= 0 && k <= 5).length;
				if (foundBasicElements === 6 && result.defaultPartyId !== null) {
					debugLog("All elements and default party mapped. Breaking early.");
					break; // 全部找齊了，提早結束迴圈，省下後續的 API 請求時間！
				}
			}
		} catch (error) {
			debugLog("buildElementPartyMap error: " + error);
		}
		return result;
	}
	/**
	...
	async function fetchOwnedStamps() {
		try {
			if (!_httpClient) { debugLog("HTTP connection not initialized"); return; }
			const stampsRes = await _httpClient.get({url: `${kh.env.urlRoot}/a_stamps`}, "unblock");
	...
	async function fetchAllGachaPools() {
		try {
			if (!_httpClient) {debugLog("HTTP connection not initialized"); return;}
			const gachaRes = await _httpClient.get({url: `${kh.env.urlRoot}/gacha_categories`}, "unblock");
	...
	async function robotDailyStart() {
		try {
			...
			//每日免費水晶
			const coinShopRes = await _httpClient.get({url: `${kh.env.urlRoot}/shop/1`}, "unblock");
	...
	async function robotUnionStart() {
		try {
			...
			//查詢活動關卡資訊
			const battleRes = await _httpClient.get({url: `${kh.env.urlRoot}/a_battles`,json: {kind: "event", event_id: _unionEventId}}, "unblock");
	...
	async function robotEpicStart() {
		try {
			...
			for (const [elementStr, quests] of Object.entries(epicData)) {
				const unlockedQuests = quests.filter(quest => quest.is_unlocked === true);
				//查表取得克制屬性與對應隊伍
				const element = parseInt(elementStr, 10);
				...
				for (const quest of unlockedQuests) {
					...
					if (unfinished) {//加入執行列表
						_epicQuests.push({
							...
							targetElement: targetElement,//直接寫入已查好的克制屬性
							partyId: partyId,//直接寫入已查好的隊伍ID
	...
	async function joinRescueRaid(rescueCode) {
		try {
			...
			const rescueRes = await _httpClient.get({
				...
				json: {kind: "raid_rescue_code",rescue_code: rescueCode}
			}, "unblock");
			//數量檢查
	...
	async function launchRaidBattle(missionQuest) {
		try {
			...
			//發送開始關卡請求
			const response = await _httpClient.post({url: missionQuest.url,json: missionQuest.json}, "unblock");
			const cannotProgressInfo = response?.body?.cannot_progress_info;
			if (cannotProgressInfo) {
				await showBattleFail(cannotProgressInfo);
				return false; 
	...
	async function executeLuckGacha() {
		try {
			...
			//檢查魔寶石數量
			const currencyRes = await _httpClient.get({url: `${kh.env.urlRoot}/a_players/me/currency`}, "unblock");
			const stoneAmount = currencyRes?.body?.stone || 0;
			if (stoneAmount < 330) {
				debugLog(`stone balance too low (${stoneAmount}/330). aborting.`);
				return;
			} 
			//find the gacha_id
			const cat2Res = await _httpClient.post({url: `${kh.env.urlRoot}/gacha_category/${luckCategoriesId}`}, "unblock");
			const gachaIdArray = cat2Res.body?.gacha_information?.map(info => info.gacha_id) || [];
			for (const gachaId of gachaIdArray) {
				try {
					await _httpClient.post({url: `${kh.env.urlRoot}/a_gacha/normal`,json: { gacha_id: gachaId }}, "unblock");
				} catch (error) {
	...
	async function executeFreeGacha() {
		try {
			...
			//獲取轉蛋分類
			const freeRes = await _httpClient.get({url: `${kh.env.urlRoot}/gacha_categories`}, "unblock");
			...
			//開始抽轉蛋
			for (let i = 0; i < 11; i++) {
				const catRes = await _httpClient.post({url: `${kh.env.urlRoot}/gacha_category/${free10CategoryId}`}, "unblock");
	...
	async function executeExpiringRaidGacha() {
		try {
			...
			//取得過期但可領獎的活動
			const eventRes = await _httpClient.get({url: `${kh.env.urlRoot}/a_banners/event_after_period`}, "unblock");
			...
			//獲取該活動的抽獎券數量
			const eventDetailRes = await _httpClient.get({url: `${kh.env.urlRoot}/a_events/raid_event/${eventId}`}, "unblock");
			...
			while (eventTicketsCount > 0) {
				...
				//抽抽
				let itemRes = await _httpClient.post({url: `${kh.env.urlRoot}/a_gacha/event`,json: {gacha_id: gachaId}}, "unblock");
				...
				//還有剩餘的券, 等待一下再抽
				if (eventTicketsCount > 0) await sleep(200);
			}
			magicJewels = magicJewels * 5;
			debugLog("magic jewels: " + magicJewels);
		} catch (error) {
	...
	async function autoPurchaseShopItems() {
		...
		try {
			...
			//75004 伏魔殿勳章,神龍眼
			const eventShopRes = await _httpClient.get({url: `${kh.env.urlRoot}/shop/7`}, "unblock");
			...
			//神龍眼
			//41010 神龍眼碎片
			const draconicShopRes = await _httpClient.get({url: `${kh.env.urlRoot}/shop/10`}, "unblock");
			...
			//93049 道具,魔導秘本[封魔]
			//93050 道具,魔導秘本[星辰]
			const labroShopRes = await _httpClient.get({url: `${kh.env.urlRoot}/shop/16`}, "unblock");
			...
			//201003 真哀聖結晶
			//201005 真勇聖結晶
			const awakenshopRes = await _httpClient.get({url: `${kh.env.urlRoot}/shop/21`}, "unblock");
			...
			//幻獸寶珠
			//61011 神龍眼碎片
			const eidolonshopRes = await _httpClient.get({url: `${kh.env.urlRoot}/shop/6`}, "unblock");
	...
	async function refillApIfNeeded() {
		try {
			const apRes = await _httpClient.get({url: `${kh.env.urlRoot}/a_players/me/quest_points`}, "unblock");
	...
	async function refillBpIfNeeded() {
		try {
			const bpRes = await _httpClient.get({url: `${kh.env.urlRoot}/a_players/me/quest_points`}, "unblock");
	...
	async function battleRaidPointsDelay() {
		try {
			...
			if (_questType === "raid" || _questType === "raid_event") {
				...
				if (bcrrp) {
					const bcrrpProto = Object.getPrototypeOf(bcrrp);
					bcrrpProto.COMMAND_DURATION = 0;
					bcrrpProto.FADE_IN_SPEED = 0.1;
					bcrrpProto.FADE_OUT_SPEED = 0.1;
					bcrrpProto.VISIBLE_DURATION = 100;
					_raidPointsDelayOk = true;
					debugLog("hook BattleCommandReceiveRaidPoints ok");
				} else {
					debugLog("no BattleCommandReceiveRaidPoints!");
				}
			}
		} catch (error) {
	...
	/**
	 * @description 偵測按下攻擊按鍵
	 */
	async function attackButtonHook() {
		try {
			if (_attackButtonHookOk) return;

			const battleWorld = kh.createInstance("battleWorld");
			const btnAttack = battleWorld?.battleUI?.AttackButton;
			if (btnAttack) {
				const btnAttackProto = Object.getPrototypeOf(btnAttack);
				const originalSimulateAttack = btnAttackProto.simulateAttack;
				btnAttackProto.simulateAttackRaw = btnAttackProto.simulateAttack;
				btnAttackProto.simulateAttack = function () {
					const result = originalSimulateAttack.call(this);
					_playerActionTime = new Date();//更新閒置檢查時間
					return result;
				}
				_attackButtonHookOk = true;
				debugLog("hook AttackButton ok");
			} else {
				debugLog("no AttackButton");
			}
		} catch (error) {
			debugLog("attackButtonHook: " + error);
		}
	}
	/**
	...
	async function onBattleStart() {
		try {
			...
			if (_battleId === currentBattleId) {
				...
				if (questType && questType === "raid") await createRankingUI();
			} else {
				_battleId = currentBattleId;//更新戰鬥ID
				if (!await waitForAttackButton()) return;//等待場景完成
				await attackButtonHook();//取得攻擊開始事件
				...
				//點擊攻擊,取得當前自動攻擊設定狀態(0:手動,1:綠,2:紅)
	...
	async function processAutoBattleTurn() {
			...
			for (let i = 0; i < characterList.length; i++) {
				...
				for (const skill of skills) {
					...
					if (character.isJob) {
						...
						//減CT技
						if (customPriority === 70 || customPriority === 71) {
							if (!hasLowerPriorityReadySkills(character, skill, skills)) {
		 ...
		 */
		function updateBeethovenMelodyState() {
			try {
		...
		/**
		 * @description 檢查技能是否處於可施放狀態
		 */
		function isSkillReady(skill) {
			const data = skill?._abilityData;
			return Boolean(data && data.ready && !data.is_banned && data.is_condition_satisfy !== false);
		}
		/**
		 * @description 取得技能的計算優先級
		 */
		function getSkillPriority(character, skill) {
			const skillIdx = skill._index;
			if (character.isJob) {
				const soulPriority = SOUL_SKILL_PRIORITIES[character.id]?.[skillIdx];
				if (soulPriority !== undefined) return soulPriority;
			} else {
				const charPriority = CHARACTER_SKILL_PRIORITIES[character.id]?.[skillIdx];
				if (charPriority !== undefined) return charPriority;
			}
			const color = skill?._abilityData?.color || "unknown";
			return SKILL_COLOR_PRIORITIES[color] || 99;
		}
		/**
		 * @description 檢查同角色的其他技能中，是否有優先級小於目前技能且 Ready 的技能
		 */
		function hasLowerPriorityReadySkills(character, currentSkill, skills) {
			const currentPriority = getSkillPriority(character, currentSkill);
			return skills.some(otherSkill => {
				if (otherSkill._index === currentSkill._index || !isSkillReady(otherSkill)) return false;
				const otherPriority = getSkillPriority(character, otherSkill);
				return otherPriority < currentPriority;
			});
		}
		/**
		 * @description 檢查全隊中是否有除指定技能外的其他可用技能
		 */
		function hasOtherReadySkillsInTeam(currentCharIndex, currentSkillIndex) {
			const characterList = battleWorld.characterList || [];
			const abilityList = battleWorld.characterAbilityList || [];

			for (let i = 0; i < characterList.length; i++) {
				const char = characterList[i];
				if (!char || char.hp === 0 || char.isJob) continue;

				const skills = abilityList[i];
				if (!Array.isArray(skills)) continue;

				for (const skill of skills) {
					if (i === currentCharIndex && skill._index === currentSkillIndex) continue;
					if (isSkillReady(skill)) return true;
				}
			}
			return false;
		}
		 ...
		 */
		function determineBeethovenTargetColors() {
			try {
				const characterList = battleWorld.characterList || [];
				const abilityList = battleWorld.characterAbilityList || [];
				const availableColors = { yellow: 0, blue: 0, green: 0, red: 0 };//統計目前可用的技能顏色數量
				for (let i = 0; i < characterList.length; i++) {
					const character = characterList[i];
					if (!character || character.hp === 0 || character.isJob) continue;
					const skills = abilityList[i];
					if (!Array.isArray(skills)) continue;
					for (const skill of skills) {
						if (!isSkillReady(skill)) continue;
						const priority = getSkillPriority(character, skill);
						const color = skill._abilityData?.color;
						if (!(color in availableColors)) continue;
						//減CT技(個人)的處理
						if ((priority === 70 || priority === 71) && hasLowerPriorityReadySkills(character, skill, skills)) {
							continue;
						}
						//減CT技(全部)的處理
						if (priority === 80 && hasOtherReadySkillsInTeam(i, skill._index)) {
							continue;
						}
